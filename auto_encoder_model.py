...
class Autoencoder(nn.Module):
    def __init__(self, input_dim, latent_dim=50,sigma = 0.5):
...
def create_pairs(orig_latent, noisy_latent, labels, num_neg_pairs=20):
...
def main():
    ...
    if pca_noise_flag:
        ...
        _, data_noisy = pca_model.create_pca_data(data_orig,d)
    ...
    plt.legend()
    plt.show()
    plt.savefig('plots_and_images/loss_vs_epochs_autoencoder_mask_noise'+'.png')

    # Save model and relevant parameters
    if pca_noise_flag:
        file_name = 'models/encoded_model_and_data_pca_noise.pt'
    elif mask_flag:
        file_name = 'models/encoded_model_and_data_mask_noise.pt'
    else:
        file_name = 'models/encoded_model_and_data_no_noise.pt'
